...
test('admin paging is bounded and integral', () => {
  ...
  for (const input of [{ limit: 0 }, { limit: 101 }, { limit: 1.5 }, { offset: -1 }])
    assert.throws(
      () => page(input),
...
test('admin profile patches preserve omitted PII fields and exclude PII from audit metadata', async () => {
  ...
  const pool = {
    query: async (sql: string, values: unknown[] = []) => {
      calls.push({ sql, values });
      if (sql.startsWith('update users')) return { rowCount: 1, rows: [{ id: 'user' }] };
      if (sql.includes('from users u where'))
        return { rowCount: 1, rows: [{ id: 'user', email: 'u@example.com', balance: 0 }] };
      return { rowCount: 0, rows: [] };
    },
  };
  const repository = new PostgresAdminDataRepository(pool as never);
  await repository.updateUser('user', 'admin', {
    firstName: 'Ada',
    address: { line1: '1 Private Lane', countryCode: 'US' },
  });
...
test('admin credit adjustment rejects reserved and invalid ledger kinds', async () => {
  const repository = new PostgresAdminDataRepository({
    connect: async () => {
      throw Error('must reject before database access');
    },
  } as never);
  for (const kind of ['usage', 'purchase', 'initial_verified_account_credit', 'invalid']) {
    await assert.rejects(
      () =>
        repository.adjustCredits('user', 'admin', { delta: 1, note: 'test', kind: kind as never }),
      (error: unknown) => error instanceof HttpError && error.statusCode === 400,
    );
  }
});
...
test('admin enum filters normalize blanks, cast safely, and reject invalid values', async () => {
  ...
  await assert.rejects(
    () => repository.users({ lifecycle: 'unknown' }),
    (e: unknown) => e instanceof HttpError && e.statusCode === 400,
  );
  await assert.rejects(
    () => repository.units({ status: 'unknown' }),
    (e: unknown) => e instanceof HttpError && e.statusCode === 400,
  );
  await assert.rejects(
    () => repository.samples({ source: 'unknown' }),
    (e: unknown) => e instanceof HttpError && e.statusCode === 400,
  );
});
...
test('admin report, sample, and credit sorting is whitelisted and deterministic', async () => {
  ...
  await assert.rejects(
    () => repository.samples({ sortBy: 'drop table' }),
    (e: unknown) => e instanceof HttpError && e.statusCode === 400,
  );
  await assert.rejects(
    () => repository.creditEntries({ sortDirection: 'sideways' }),
    (e: unknown) => e instanceof HttpError && e.statusCode === 400,
  );
});
...
test('admin profile update never updates email', async () => {
  ...
  await new PostgresAdminDataRepository(pool as never).updateUser('user', 'admin', {
    firstName: 'Ada',
