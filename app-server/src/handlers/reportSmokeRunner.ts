...
async function seedSmokeReport(pool:Pool, rawPrefix:string, reportPrefix:string):Promise<Seed> {
  ...
  try {
    ...
    await client.query(`insert into sample_upload_batches(id,owner_id,psiu_unit_id,idempotency_key,request_fingerprint,source) values($1,$2,$3,$4,$5,'manual_file')`,[batchId,ownerId,unitId,`smoke-${requestId}`,createHash('sha256').update(`smoke-batch:${requestId}`).digest('hex')]);
