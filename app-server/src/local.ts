...
export function macOsCredentialStore(): CredentialStore {
  ...
  return {
    load() {
      const result = spawnSync('security', ['find-generic-password', '-s', bridgeService, '-a', bridgeAccount, '-w'], { encoding: 'utf8' });
      return result.status === 0 ? result.stdout.trim() || undefined : undefined;
    },
    save(value) {
      const result = spawnSync('security', ['add-generic-password', '-U', '-s', bridgeService, '-a', bridgeAccount, '-w', value], { encoding: 'utf8' });
      if (result.status !== 0) throw new Error('Unable to store PSIU pairing in the OS credential store.');
    },
  };
}
...
export function promptMacOsPairing(run: typeof spawnSync = spawnSync): { username: string; password: string } {
  ...
  const username = ask('PSIU username');
  const password = ask('PSIU password', true);
  if (!username || !password) throw new Error('PSIU username and password are required.');
  return { username, password };
}

export async function pairLocalBridge(baseUrl: string, dependencies: { store?: CredentialStore; prompt?: () => { username: string; password: string }; validate?: (value: string) => Promise<string> } = {}) {
  const normalized = await (dependencies.validate ?? validatePsiuBaseUrl)(baseUrl);
  const prompt = dependencies.prompt ?? (() => promptMacOsPairing());
  const { username, password } = prompt();
  const authorization = `Basic ${Buffer.from(`${username}:${password}`).toString('base64')}`;
  (dependencies.store ?? macOsCredentialStore()).save(JSON.stringify({ baseUrl: normalized, authorization } satisfies BridgePairing));
}

export async function createLocalServer(dependencies: { authorization?: string; environment?: NodeJS.ProcessEnv; fetchImplementation?: typeof fetch; store?: CredentialStore; secrets?: PsiuCredentialClient } = {}): Promise<Server> {
  const environment = dependencies.environment ?? process.env;
  ...
  const pairing = developmentHarness
    ...
    // Explicit dependency injection exists for tests and does not read environment credentials.
    : dependencies.authorization !== undefined
      ? { baseUrl: environment.PSIU_BASE_URL ?? 'http://psiu.local', authorization: dependencies.authorization }
      : customerPairing(dependencies.store ?? defaultCustomerStore());
  // Target validation occurs before any request can receive the paired authorization.
  const psiuBaseUrl = await validatePsiuBaseUrl(pairing.baseUrl);
  ...
  return createServer(async (request, response) => {
    ...
    if (request.method === 'POST' && url.pathname === '/psiu/capture') try {
      const { running } = await parseCaptureRequest(request);
      await requestPsiu(psiuBaseUrl, authorization, '/api/sampling', fetchImplementation, { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ running }) });
      return forwardPsiu(response, '/status', psiuBaseUrl, authorization, fetchImplementation, cors);
    } catch (error) { return sendPsiuError(response, error, cors); }
    return sendJson(response, 404, { message: 'Route not found.' }, cors);
  });
}

function defaultCustomerStore(): CredentialStore {
  return process.platform === 'darwin' ? macOsCredentialStore() : { load: () => undefined, save: () => { throw new Error('Customer local bridge requires a supported OS credential-store adapter.'); } };
}

function customerPairing(store: CredentialStore): { baseUrl: string; authorization?: string } {
  // Unpaired mode exposes only unauthenticated discovery; protected PSIU routes fail upstream.
  const value = store.load();
  if (!value) return { baseUrl: 'http://psiu.local' };
  let pairing: unknown;
  try { pairing = JSON.parse(value); } catch { throw new Error('Stored PSIU pairing is invalid. Pair the bridge again.'); }
  if (!pairing || typeof pairing !== 'object' || Array.isArray(pairing)) throw new Error('Stored PSIU pairing is invalid. Pair the bridge again.');
  const record = pairing as Record<string, unknown>;
  if (typeof record.baseUrl !== 'string' || typeof record.authorization !== 'string' || !record.authorization.trim()) throw new Error('Stored PSIU pairing is invalid. Pair the bridge again.');
  return { baseUrl: record.baseUrl, authorization: record.authorization };
}

export async function validatePsiuBaseUrl(value: string): Promise<string> {
  let url: URL;
  try { url = new URL(value); } catch { throw new Error('PSIU target is invalid.'); }
  if (!['http:', 'https:'].includes(url.protocol) || url.username || url.password || url.pathname !== '/' || url.search || url.hash) throw new Error('PSIU target must be an origin without credentials, path, query, or fragment.');
  const hostname = url.hostname.toLowerCase();
  if (hostname !== 'psiu.local') {
    const addresses = isIP(hostname) ? [hostname] : (await lookup(hostname, { all: true })).map(result => result.address);
    if (!addresses.length || addresses.some(address => !isLocalAddress(address))) throw new Error('PSIU target must resolve only to a private, link-local, or loopback address.');
  }
...
async function main() {
  if (process.argv[2] === 'pair') {
    const baseIndex = process.argv.indexOf('--psiu-base-url');
    const baseUrl = baseIndex >= 0 ? process.argv[baseIndex + 1] : undefined;
    if (!baseUrl) throw new Error('Usage: npm run bridge:pair -- --psiu-base-url http://psiu.local');
    await pairLocalBridge(baseUrl);
    console.info('PSIU pairing saved locally.');
    return;
  }
  const server = await createLocalServer();
  server.listen(port, '127.0.0.1', () => console.info(`Wally local bridge listening on http://127.0.0.1:${port}`));
}
