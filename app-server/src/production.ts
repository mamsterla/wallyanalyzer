...
async function adminRoute(request:IncomingMessage,response:ServerResponse,path:string,actor:string,repo:AccountRepository,cognito:CognitoIdentityProviderClient,s3:S3Client,bucketName:string):Promise<void>{
  ...
  if(request.method==='POST'&&path.endsWith('/restore')){const x=await repo.restoreCustomer(id,actor,reqId);if(x.cognitoSubject)await cognito.send(new AdminEnableUserCommand({UserPoolId:poolId(),Username:x.email}));return sendJson(response,204);}
...
async function sampleRoute(request:IncomingMessage,response:ServerResponse,path:string,ownerId:string,samples:SampleRepository,s3:S3Client,bucketName:string):Promise<void>{
  ...
  if(request.method==='POST'&&action==='complete'){const intent=await samples.intent(ownerId,sampleId);if(!intent){const completed=await samples.sample(ownerId,sampleId);if(completed)return sendJson(response,200,{sample:completed});if(await deleteFailedUploadObject(samples,ownerId,sampleId,s3,bucketName))throw new HttpError(403,'PSIU unit is unavailable or no longer assigned; upload was rejected.');throw new HttpError(404,'Upload intent not found.');}let metadata;try{metadata=await verifyWavObject(intent,bucketName,s3);}catch(error){await deleteFailedUploadObject(samples,ownerId,sampleId,s3,bucketName);throw error;}let taggedAccepted=false;try{return sendJson(response,200,{sample:await samples.complete(ownerId,sampleId,metadata,async()=>{await setUploadLifecycleTag(s3,bucketName,intent.objectKey,'accepted');taggedAccepted=true;})});}catch(error){if(taggedAccepted)try{await setUploadLifecycleTag(s3,bucketName,intent.objectKey,'unaccepted');}catch{/* Lifecycle cleanup remains best effort after a failed database completion. */}if(error instanceof HttpError&&error.statusCode===403)await deleteUnavailableUploadObjects(s3,bucketName,[intent.objectKey]);throw error;}}
