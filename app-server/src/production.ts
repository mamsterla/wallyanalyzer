...
export function createProductionServer(dependencies: Partial<ProductionDependencies> = {}) {
  const pool = dependencies.pool ?? new Pool(databaseSettings()); const repository = dependencies.repository ?? new PostgresAccountRepository(pool);
