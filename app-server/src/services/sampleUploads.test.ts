...
test('rejects non-WAV upload intents',()=>assert.throws(()=>validateUploadBatch({idempotencyKey:'x'.repeat(16),psiuUnitId:'unit',files:[{clientFileId:'a'.repeat(16),fileName:'sample.mp3',contentType:'audio/mpeg',byteLength:44,recordedAt:new Date().toISOString()}]}),HttpError));
