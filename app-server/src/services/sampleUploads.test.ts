...
test('rejects non-WAV upload intents',()=>assert.throws(()=>validateUploadBatch({idempotencyKey:'x'.repeat(16),psiuUnitId:'unit',source:'manual_file',files:[{clientFileId:'a'.repeat(16),fileName:'sample.mp3',source:'manual_file',contentType:'audio/mpeg',byteLength:44,recordedAt:new Date().toISOString()}]}),HttpError));
test('requires observed UID only for PSIU captures',()=>assert.throws(()=>validateUploadBatch({idempotencyKey:'x'.repeat(16),psiuUnitId:'unit',source:'psiu_capture',files:[{clientFileId:'a'.repeat(16),fileName:'sample.wav',source:'psiu_capture',contentType:'audio/wav',byteLength:44,recordedAt:new Date().toISOString()}]}),HttpError));
