...
export type UploadIntentRow = { sampleId:string; fileName:string; objectKey:string; contentType:string; byteLength:number; sha256Base64?:string };
...
export async function presignUpload(intent: UploadIntentRow, bucketName:string, s3:S3Client) {
  ...
  return { sampleId:intent.sampleId, fileName:intent.fileName, uploadUrl, expiresAt:new Date(Date.now()+URL_TTL_SECONDS*1000).toISOString(), requiredHeaders:{ 'content-type':intent.contentType, ...(intent.sha256Base64 ? { 'x-amz-checksum-sha256':intent.sha256Base64 } : {}) } };
}
...
export function parseWavHeader(bytes:Uint8Array, totalBytes:number): { sampleRateHz:number; channels:number; bitsPerSample:number; durationMs:number } {
  ...
  let offset=12; let format: {channels:number;rate:number;bits:number;blockAlign:number}|undefined; let dataLength:number|undefined;
  while(offset+8<=b.length){ const id=b.toString('ascii',offset,offset+4); const length=b.readUInt32LE(offset+4); const next=offset+8+length+(length%2); if(next>b.length && id!=='data') break; if(id==='fmt '){ if(length<16||offset+24>b.length) throw new HttpError(409,'WAV format chunk is invalid.'); const tag=b.readUInt16LE(offset+8); if(tag!==1&&tag!==3) throw new HttpError(409,'Only PCM WAV files are accepted.'); format={channels:b.readUInt16LE(offset+10),rate:b.readUInt32LE(offset+12),blockAlign:b.readUInt16LE(offset+20),bits:b.readUInt16LE(offset+22)}; } if(id==='data'){dataLength=length;break;} offset=next; }
  if(!format||dataLength===undefined||format.channels<1||format.channels>8||format.rate<8000||format.rate>384000||![8,16,24,32].includes(format.bits)||format.blockAlign<1) throw new HttpError(409,'WAV header is unsupported or incomplete.');
  if(totalBytes<offset+8+dataLength) throw new HttpError(409,'WAV data chunk exceeds uploaded object length.');
