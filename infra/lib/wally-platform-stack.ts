...
export class WallyPlatformStack extends cdk.Stack {
  constructor(scope: Construct, id: string, props: WallyPlatformStackProps) {
    ...
    for(const fn of [preflightFn,finalizerFn,failedFn,requestFinalizerFn,dispatcherFn]){database.secret!.grantRead(fn);fn.addToRolePolicy(new iam.PolicyStatement({actions:['s3:GetObject','s3:GetObjectVersion'],resources:[sampleBucket.arnForObjects('raw/*'),reportBucket.arnForObjects('reports/*')]}));}
    ...
    const worker=new sfnTasks.LambdaInvoke(this,'ReportWorker',{lambdaFunction:analysisWorker,payload:sfn.TaskInput.fromObject({'requestId.$':'$.preflight.requestId','reportId.$':'$.preflight.reportId','algorithmVersion.$':'$.preflight.algorithmVersion','inputs.$':'$.preflight.inputs','effectiveAlgorithmValues.$':'$.preflight.effectiveAlgorithmValues','outputPrefix.$':'$.preflight.outputPrefix'}),payloadResponseOnly:true,resultPath:'$.worker'});
    ...
    for(const fn of [smokePreflightFn,smokeFinalizerFn,smokeFailedFn,smokeRequestFinalizerFn,smokeDispatcherFn]){smokeDatabaseSecret.grantRead(fn);fn.addToRolePolicy(new iam.PolicyStatement({actions:['s3:GetObject','s3:GetObjectVersion'],resources:[sampleBucket.arnForObjects('smoke/raw/*'),reportBucket.arnForObjects('smoke/reports/*')]}));}
    ...
    const smokeWorker=new sfnTasks.LambdaInvoke(this,'SmokeReportWorker',{lambdaFunction:smokeAnalysisWorker,payload:sfn.TaskInput.fromObject({'requestId.$':'$.preflight.requestId','reportId.$':'$.preflight.reportId','algorithmVersion.$':'$.preflight.algorithmVersion','inputs.$':'$.preflight.inputs','effectiveAlgorithmValues.$':'$.preflight.effectiveAlgorithmValues','outputPrefix.$':'$.preflight.outputPrefix'}),payloadResponseOnly:true,resultPath:'$.worker'});
