...
- Before first use, an approved operator runs the reviewed one-time bootstrap script through an SSM tunnel to create the role/database using short-lived operator credentials. No Lambda, ECS task, CodeBuild project, Step Functions workflow, or persistent IAM role receives the platform-admin database secret. The script creates or updates only `wally_report_smoke` and exits.
