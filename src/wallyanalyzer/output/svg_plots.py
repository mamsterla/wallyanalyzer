...
def render_measurement_validation_svg(
    ...
    panels = [
        ("Lag (µs)", [(lag_us, valid, "line1")]),
        ("Frequency (Hz)", [(freq_left, valid, "line1"), (freq_right, valid, "line2")]),
        ("2nd harmonic (%)", [(harm2_pct, valid, "line3")]),
        ("3rd harmonic (%)", [(harm3_pct, valid, "line4")]),
    ]

    for panel_index, (label, series_list) in enumerate(panels):
...
def _panel_svg(x: np.ndarray, y_series: list[tuple[np.ndarray, np.ndarray, str]], label: str, left: float, top: float, width: float, height: float) -> list[str]:
    valid_y_values = []
