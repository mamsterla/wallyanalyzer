...
def compile_one_measurement(
    ...
    smoothing_window = max(1, min(valid_indices.size, nrot * config.smoothing_rotations))
    ...
    ate_measured_deg = apparent_tracking_error_from_lag_deg(radius_smooth_mm, lag_smooth_s, effective_lr_um)
    ...
    distortion_model_trace = distortion_parameter(
        radius_mm=radius_smooth_mm,
        effective_length_mm=effective_length_mm,
        offset_angle_deg=offset_angle_deg,
        overhang_mm=effective_overhang_mm,
        mount_yaw_deg=mount_yaw_deg,
        cut_velocity_m_per_s=measurement.cut_velocity_m_per_s,
        angular_velocity_rad_per_s=measurement.angular_velocity_rad_per_s,
    )

    t3 = time.perf_counter()
    distortion_fit = fit_distortion_geometry(
        radius_mm=radius_smooth_mm,
        distortion_ratio=distortion_measured_ratio,
        effective_length_mm=effective_length_mm,
        offset_angle_deg=offset_angle_deg,
        overhang_guess_mm=effective_overhang_mm,
        mount_yaw_guess_deg=mount_yaw_deg,
        cut_velocity_m_per_s=measurement.cut_velocity_m_per_s,
        angular_velocity_rad_per_s=measurement.angular_velocity_rad_per_s,
        max_iter=config.distortion_fit_max_iter,
    )
    fitted_mount_yaw_deg = float(distortion_fit.x[0])
    fitted_overhang_mm = float(distortion_fit.x[1])
    t_distortion_fit = time.perf_counter() - t3
    distortion_fit_trace = distortion_parameter(
        radius_mm=radius_smooth_mm,
        effective_length_mm=effective_length_mm,
        offset_angle_deg=offset_angle_deg,
        overhang_mm=fitted_overhang_mm,
        mount_yaw_deg=fitted_mount_yaw_deg,
        cut_velocity_m_per_s=measurement.cut_velocity_m_per_s,
        angular_velocity_rad_per_s=measurement.angular_velocity_rad_per_s,
    )
    ...
    diagnostics = {
        ...
        else float(abs(effective_length_mm - effective_overhang_mm - float(actual_pivot_to_spindle_mm))),
        "timings_s": {
    ...
    return SingleCompileResult(
        ...
        distortion_fit_params=np.asarray(distortion_fit.x, dtype=float),
        distortion_fit_objective=float(distortion_fit.fun),
        distortion_fit_success=bool(distortion_fit.success),
